...
BOT_TOKEN = '8726448384:AAFlnrGl2lJYK1yHFocoQkuJpn5BMvC832E'
ADMIN_ID = [8348667414]
...
async def force_start(event):
    ...
    try:
        ...
        buttons = [
            [
                Button.url("💎 Join Channel",CHANNEL_LINK, style="primary", icon=5361734213370396027),

                Button.url("💬 Join Group",GROUP_LINK, style="primary", icon=5361734213370396027)
            ],
            [
                Button.inline("☑️ Verify Joined",data=b"verify_join", style="success", icon=5445408306669582934)
            ]
        ]
...
async def verify_join(event):
    ...
    try:
        ...
        buttons = [
            ...
            [
                Button.inline("☑️ Verify Again",data=b"verify_join", style="success", icon=5445408306669582934)
            ]
        ]
...
def get_main_menu_keyboard(user_id=None):
    buttons = [
        [Button.inline("🛍️ Gates", b"show_gates", style="primary", icon=4904936030232117798),
         Button.inline("🛠️ Tᴏᴏʟs", b"tools_menu", style="primary", icon=5361734213370396027)],
        [Button.inline("💎 My Plan", b"show_plan", style="primary", icon=4904936030232117798),
         Button.inline("⚙ Help", b"show_help", style="primary", icon=5361734213370396027)],
        [Button.url("➜ Cʜᴀɴɴᴇʟ", "https://t.me/kesavredirect", style="success", icon=5445408306669582934)]
    ]
...
async def send_hit_to_channel(card, status, response, gateway, price):
    ...
    try:
        ...
        msg = premium_emoji(f"""{status_text}
            ...
            📝 {response[:45]}
            ⏱️ {time_str}
            🍑 <a href='tg://user?id=8978049174'>KESAV</a>""")
...
💡 Mᴀᴅᴇ ʙʏ @kesav82"""
    ...
    filename = f"kesav{timestamp}.txt"
...
#gk gates
@bot.on(events.CallbackQuery(data=b"show_gates"))
async def show_commands_callback(event):
    commands_text = """📋 Gates
...
└─👆🏻 Select Gates"""
    ...
    buttons = [
        [Button.inline("➤ Charge", b"Charge1", style="primary", icon=5445408306669582934),
         Button.inline("➤ Auth", b"Auth1", style="primary", icon=5445350109862720603)],
...
🔧 <b>Hɪᴛs Mᴀɴᴀɢᴇᴍᴇɴᴛ</b>
├─ <code>/rahts ᴄʜᴀɴɴᴇʟ_ɪᴅ</code> → Sᴇᴛ ʜɪᴛs ᴄʜᴀɴɴᴇʟ
└─ <code>/onhtt</code> → Tᴏɢɢʟᴇ ʜɪᴛs ᴏɴ/ᴏғғ
└─ <code>/tux</code> → verify """
    buttons = [[Button.inline(" Bᴀᴄᴋ", b"main_menu", style="danger", icon=5445365692004071819)]]
...
#inside gates gk
@bot.on(events.CallbackQuery(data=b"Charge1"))
async def tools_menu_callback(event):
    user_id = event.sender_id
    
    tools_text = """🛠️ <b>Charge </b>

1. <b>𝘼𝙪𝙩𝙤 𝙎𝙝𝙤𝙥𝙞𝙛𝙮</b>
├─ <code>/cc</code>  → Cʜᴇᴄᴋ sɪɴɢʟᴇ ᴄᴀʀᴅ
                        (ᴄᴄ|ᴍᴍ|ʏʏ|ᴄᴠᴠ)
├─ <code>/chk</code> → Mᴀss ᴄʜᴇᴄᴋ ғʀᴏᴍ .ᴛxᴛ ғɪʟᴇ
                        (ʀᴇᴘʟʏ ᴛᴏ ғɪʟᴇs)"""

    buttons = [
        [Button.inline("Bᴀᴄᴋ", b"show_gates", style="danger", icon=5445365692004071819)]
    ]
    
    await event.edit(premium_emoji(tools_text), buttons=buttons, parse_mode='html')


@bot.on(events.CallbackQuery(data=b"Auth1"))
async def tools_menu_callback(event):
    user_id = event.sender_id
    
    tools_text = """‼️ <b>Coming Soon ‼️ </b>

1. <b>Stripe Auth</b>
├─ <code>/st</code>  → Cʜᴇᴄᴋ sɪɴɢʟᴇ ᴄᴀʀᴅ
                        (ᴄᴄ|ᴍᴍ|ʏʏ|ᴄᴠᴠ)
├─ <code>/mst</code> → Mᴀss ᴄʜᴇᴄᴋ ғʀᴏᴍ .ᴛxᴛ ғɪʟᴇ
                        (ʀᴇᴘʟʏ ᴛᴏ ғɪʟᴇs)"""
...
@bot.on(events.CallbackQuery(data=b"show_plan"))
async def tools_menu_callback(event):
...
@bot.on(events.NewMessage(pattern='/rahts'))
async def set_hits_channel(event):
    ...
    try:
        parts = event.raw_text.split()
        if len(parts) != 2:
            await event.reply(premium_emoji("📝 Usᴀɢᴇ: <code>/rahts -1001234567890</code>"), parse_mode='html')
            return
...
@bot.on(events.NewMessage(pattern='/onhtt'))
...
async def test_private_channel(event):
    try:
        ...
        await event.reply("✅ Message successfully sent to channel.")
